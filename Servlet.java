...
public class Servlet extends HttpServlet{
	...
	/**
	 * @param request Contains information on the client requesting the information from the server
	 * @param response Contains the information that the server can use to send informatino back
	 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		else if(array[0].equals("DELCAT")){
			try{
				String statement = "UPDATE eventData SET category='None' WHERE userID='" + array[1] + "' AND category='" + array[2] + "'";
				Statement stmt = con.createStatement();
				boolean success = stmt.execute(statement);
		...
		else if(array[0].equals("SEARCHCAT")){
			try{
				String statement = "SELECT DISTINCT category FROM eventData WHERE userID='" + array[1] + "'";
				Statement stmt = con.createStatement();
				...
				while(rs.next()){
					String send = " " + rs.getString("category");
	...
	/**
	 * 
	 * @param e - Contains the exception to be sent to the user's phone
	 * @param out - Copy of the output stream to send the message on
	 */
